...
class ModelData() :
    ...
    def loadFile( self, inputFile ) :
        ...
        for (index, line) in enumerate(lines, start = 1) :
            ...
            if len(line) > 0 :
                if line[0]=="#":
                    ...
                    if len(m_valuelist) > 0:
                        tupvalue = tuple(m_valuelist)
                        self.m_Patches.append(tupvalue)
                    ...
                    for item in line :
                        if item != " ":
                            ...
                            if number != "":
                                ...
                                number = ""
                    if len(m_valuelist) > 0:
                        # print("Y value",m_valuelist[1])
                        if m_valuelist[0] <= self.xmin:
                            if(self.xmax <= self.xmin and self.xmin != float('inf')):
                                self.xmax = self.xmin
                        ...
                        if m_valuelist[1] <= self.ymin:
                            if(self.ymax <= self.ymin and self.ymin != float('inf')):
                                self.ymax = self.ymin
                        ...
                        if m_valuelist[2] <= self.zmin:
                            if(self.zmax <= self.zmin and self.zmin != float('inf')):
                                self.zmax = self.zmin
        ...
        print("Bounding Box:",self.m_BoundingBox)
    ...
    def transformXYZ( self, x,y,z, doPerspective=False, doEuler=False ) :
        transformedvertex = []
        xcomp = x
        ycomp = y
        zcomp = z
