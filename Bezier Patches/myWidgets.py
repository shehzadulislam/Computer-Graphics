...
class cl_canvas_frame :
  def __init__( self, master ) :
    ...
    self.canvas = tk.Canvas(
      master.ob_root_window, width=650, height=650, bg='#99ff99' )
...
class cl_menu :
  def __init__( self, master ) :
    ...
    self.dummymenu.add_checkbutton(label="Euler Rotation", onvalue=1, offvalue=False, variable= self.master.doEuler  ,command = lambda : self.menu_callback( 'settings>Euler Rotation' ))
...
class cl_toolbar :
  def __init__( self, master ) :
    ...
    self.button = tk.Button( self.toolbar, text = 'Perspective', width = 8, command = self.toolbar_perspective_callback )
    self.button.pack( fill = tk.X, side = tk.LEFT, expand=True, padx = 2, pady = 2 )
    ...
    self.button = tk.Button( self.toolbar, text = 'Reset', width = 6, command = self.toolbar_reset_callback )
    self.button.pack( fill = tk.X, side = tk.LEFT, expand=True, padx = 2, pady = 2 )
    self.button = tk.Button( self.toolbar, text = 'Load', width = 6, command = self.toolbar_load_callback )
    self.button.pack( fill = tk.X, side = tk.LEFT, expand=True, padx = 2, pady = 2 )
    self.button = tk.Button( self.toolbar, text = 'Draw', width = 6, command = self.toolbar_draw_callback )
    self.button.pack( fill = tk.X, side = tk.LEFT, expand=True, padx = 2, pady = 2 )
    self.toolbar.pack( side = tk.TOP, fill = tk.X )

  def toolbar_resolution_callback( self ) :
    resolution = simpledialog.askinteger("Resolution", "Resolution", initialvalue=self.master.Resolution,minvalue = 4, maxvalue = 2000)
    if resolution is None:
      self.master.statusBar_frame.set( 'Resolution remains same ' + str(self.master.Resolution) )
    else:
      self.master.Resolution = resolution
      self.master.statusBar_frame.set( 'Resolution changed to ' + str(self.master.Resolution) )
  ...
  def toolbar_roll_callback( self ) :
    roll = simpledialog.askfloat("ϕ", "Roll", initialvalue=self.master.roll,minvalue=-360.0, maxvalue=360.0)
    if roll is None:
      self.master.statusBar_frame.set( 'ϕ remains same ' + str(self.master.roll) )
    else:
      self.master.roll = roll
      self.master.statusBar_frame.set( 'ϕ changed to ' + str(self.master.roll) )

  def toolbar_pitch_callback( self ) :
    pitch = simpledialog.askfloat("θ", "Pitch", initialvalue=self.master.pitch,minvalue=-360.0, maxvalue=360.0)
    if pitch is None:
      self.master.statusBar_frame.set( 'θ remains same ' + str(self.master.pitch) )
    else:
      self.master.pitch = pitch
      self.master.statusBar_frame.set( 'θ changed to ' + str(self.master.pitch) )

  def toolbar_yaw_callback( self ) :
    yaw = simpledialog.askfloat("ψ", "Yaw", initialvalue=self.master.yaw,minvalue=-360.0, maxvalue=360.0)
    if yaw is None:
      self.master.statusBar_frame.set( 'ψ remains same ' + str(self.master.yaw) )
    else:
      self.master.yaw = yaw
      self.master.statusBar_frame.set( 'ψ changed to ' + str(self.master.yaw) )
  ...
  def toolbar_draw_callback( self ) :
    if self.master.model is None:
        ...
        if(len(w) == 0 and len(v) == 0):
            ...
            self.master.model.getCenter()
            if self.master.doEuler.get() == 1:
              self.master.model.specifyEulerAngles( self.master.roll, self.master.pitch,self.master.yaw )
